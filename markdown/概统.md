...
4. $$\frac{\bar{X}-\bar{Y}-(\mu_1-\mu_2)}{\sqrt{\frac{1}{n}+\frac{1}{m}}S_T}\sim t_{m+n-2}$$
...
$$\frac{1}{n}\sum_{j=1}^{n}X_j^k=E(X^k)$$

或者

$$\frac{1}{n}\sum_{j=1}^{n}(X_j-\bar{X})^k=E(X-\alpha_1)^k\quad\alpha_1=E(X)$$
